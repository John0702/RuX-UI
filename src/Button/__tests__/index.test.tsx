...
describe('Button组件', () => {
  ...
  it('能够正确渲染默认样式的按钮', () => {
    ...
    const buttonElement = getByRole('button');
    expect(buttonElement.classList.contains('rux-btn')).toBe(true);
  });

  it('能够正确渲染主要样式的按钮', () => {
    const { getByRole } = render(<Button type="primary">主要按钮</Button>);
    const buttonElement = getByRole('button');
    expect(buttonElement.classList.contains('rux-btn-primary')).toBe(true);
  });
...
// 该单元测试代码分别测试了按钮基础渲染，渲染默认样式，渲染主要样式以及点击测试功能。
